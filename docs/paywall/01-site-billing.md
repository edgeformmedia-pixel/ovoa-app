...
Repo: `C:\Users\thoma\OneDrive\Documents\GitHub\ovoa-team` (TanStack Start on the Cloudflare Worker `ovoa-site`, with D1).
...
5. Storage: write a D1 migration in `migrations/`. Add `tier` to members and create `band_orders`.
...
- Don't touch the live Stripe keys. Test mode only.

## Finish
- Commit with plain-sentence messages (look at `git log` for the style) and push `main`.
- Deploy the site with the admin@ovoa.ai profile: `XDG_CONFIG_HOME=C:/Users/thoma/.wrangler-ovoa npm run db:migrate`, then the same with `npm run deploy`.
- Leave a short list of the Stripe steps the user must do; agent 06 folds it into `setup.md`.
