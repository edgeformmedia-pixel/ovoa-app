...
// and the data needed to answer it go to Z.ai (GLM) to write replies, and to
// Google Gemini when GLM is down or for web searches; Deepgram turns replies
// into speech from the text alone. The app shows that screen (app/consent.tsx)
// and posts here when they agree; users.ai_consent_at and ai_consent_version
// (migration 0041) keep the answer.
//
// Two places ask:
...
//     /chat/resume, Siri) and /voice/speak (Deepgram). Without this a turn
//     refused by the gate would still have its one-sentence answer voiced by
//     Deepgram.
//
...
/** The wording of the consent screen now. Bump it with the app's lib/consent.ts CONSENT_VERSION when the screen changes. */
 ...
 * the model call itself, and answers needs_consent from there.
 */
const CONSENT_ROUTES: RegExp[] = [/^\/(chat|chat\/resume|siri)$/, /^\/voice\/speak$/];
...
consentRoutes.post("/me/consent", async (c) => {
  ...
  const version = Math.min(parsed.data.version, AI_CONSENT_VERSION);
