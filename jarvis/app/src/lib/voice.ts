 ...
 * background. `wake` false (a plan without the hands-free wake word) listens
 * in open mode: every sentence is for the assistant, as it is for dictation
 * and setup. `fillers` false: no "Let me look into that" while it thinks
 * (setup, where the reply is a scripted question). `answers`: each turn
 * answers a question (setup), so a one-word answer isn't taken for the
 * question's echo (turnGate.ts).
 *
